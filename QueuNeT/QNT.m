...
while sigLevel_main > dSignificant_level
    clear s_p2
    tic_1 = tic;
    set_param('QueuingSys/Constant','Value','1') % set the model to get the time value from the first processor
    simtime = sim('QueuingSys','SaveTime','on','TimeSaveName','tout');% sampling from the model
    ...
    if simtime.tout(end)< 100
        ...
        set_param('QueuingSys/Constant','Value','2')% set the model to get the time value from the second processor
        for k = 1 : c
